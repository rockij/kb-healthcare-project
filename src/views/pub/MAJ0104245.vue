            ...
            <ul class="evt-attend-chk-list">
              <li v-for="(item, index) in stampList" :key="index">
                <button type="button" class="stamp" @click="evtAttend(index)">
                  ...
                  <span
                    v-if="item.isChk && !item.point.state && !isPerfect.value"
                    class="stamp-chk"
                  ...
                  <span
                    v-if="
                      item.isChk && isPerfect && index + 1 == stampList.length
                    "
                    class="stamp-perfect"
                    ><span class="offscreen">perfect</span></span
                  >
                </button>
              </li>
            </ul>
    ...
    <v-dialog v-model="dialog" class="event-dialog">
      <v-card>
        ...
        <v-card-actions class="d-flex">
          <v-btn class="btn-event-close" @click="dialog = false"
            ><span class="offscreen">팝업닫기</span></v-btn
          >
        </v-card-actions>
      </v-card>
    </v-dialog>
