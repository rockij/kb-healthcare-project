...
;(function ($, window, document, d3, undefined) {
  ...
  // minified (cokespecially when cokboth are regularly referenced in your plugin).
  ...
  $.extend(Plugin.prototype, {
    ...
    helper: {
      ...
      calculateTimeFromMinutes: function (min) {
        ...
        var minutes = parseInt((min % 60) / 10) * 10
      ...
      createTimeInfoObject: function (data) {
        ...
        startTimeInfo = this.calculateTimeFromMinutes(
          angleToSegments(data.aAngle) * 5
        )
        endTimeInfo = this.calculateTimeFromMinutes(
          angleToSegments(data.eAngle) * 5
        )
    ...
    init: function () {
      ...
      var angleToFiveMinuteScale = d3
        .scaleLinear()
        .range([0, 360])
        .domain([0, 288]) //24 Hours have 288 5 Min. Segments
      ...
      var drawHandles = function () {
        var handlerContainer = handles
          .selectAll('.handlercontainer')
          .data(_this.helper.getData())
        // console.log(handlerContainer)
        var circles = handlerContainer
          .enter()
          .append('g')
        ...
        circles
          ...
          .attr('fill', function (d, i) {
            if (d.label == 'a') return _this.settings.handleFillColorStart
            else return _this.settings.handleFillColorEnd
          })
          .attr('id', function (d) {
            return d.label
          })
        ...
        circles
          .append('image')
          // .attr('href', function(d, i) { if(d.label == "a") return '../../img/content/ic-sleep-night.svg'; else return '../../img/content/ic-sleep-morning.svg' })
          .attr('href', function (d, i) {
            if (d.label == 'a') return '../../common/image/icon-sleep-night.svg'
            else return '../../common/image/icon-sleep-morning.svg'
          })
          //.attr('width', '30')
          //.attr('height', '30')
      ...
      this.writeTimeInfo(
        _this.helper.createTimeInfoObject({
          a: a,
          e: e,
          aAngle: startAngle,
          eAngle: endAngle
      ...
      indicatorArc = holder
        ...
        .attr(
          'transform',
          'translate(' +
            (this.settings.width / 2 - 20) +
            ',' +
            (this.settings.height / 2 - 20) +
            ')'
        )
        ...
        .attr('fill', function (d, i) {
          return _this.settings.accentColor
        })
    ...
    drawClockTicks: function () {
      ...
      ticks = svg
        ...
        .attr('transform', function (d) {
          return (
            ...
            outerRadius +
            ',0)'
          ) //move 0 -90 degree -> 0 must be top position
        })
      ...
      ticks
        ...
        .attr('y1', 0)
        .attr('x2', function (d) {
          if (d.label || d.angle == 0) return 10
          else return 5
        })
        .attr('y2', 0)
